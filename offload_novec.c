...
void sini(const float * restrict in, float * restrict out, int i);

//This function will be compiled only for the target device (mic)
void some_work(const float * restrict in, float * restrict out)
...
#pragma omp target map(to:in_array[:SIZE]) map(from:out_array[:SIZE])
       {
