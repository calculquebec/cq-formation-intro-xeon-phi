...
!DIR$ OFFLOAD_TRANSFER target(mic) nocopy(in_array, out_array : length(SIZE) alloc_if(.TRUE.) free_if(.FALSE.))
...
!DIR& in(in_array: length(SIZE) alloc_if(.FALSE.) free_if(.FALSE.) )
!DIR& out(out_array: length(SIZE) alloc_if(.FALSE.) free_if(.FALSE.) )
...
!DIR$ OFFLOAD_TRANSFER target(mic) nocopy(in_array, out_array : length(SIZE) alloc_if(.FALSE.) free_if(.TRUE.) )
